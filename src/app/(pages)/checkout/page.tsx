...
export default function CheckoutPage() {
  const { items, total, removeItem, clearCart, updateItemQuantity } = useCart();
  const [orderPlaced, setOrderPlaced] = useState(false);

  const VAT_RATE = 25.5; // VAT percentage
  ...
  if (orderPlaced) {
    return (
      <div className="max-w-4xl mx-auto p-6 text-center">
        <h1 className="text-2xl font-bold mb-4">Kiitos tilauksestasi!</h1>
        <p>Tilauksesi on vastaanotettu ja käsittely on aloitettu.</p>
        <Link href="/" className="mt-4 inline-block px-6 py-2 bg-black text-white rounded-md">
          Palaa etusivulle
        </Link>
      </div>
    );
